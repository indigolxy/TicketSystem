...
private:
    ...
    int num;
    int price;
    int status; // 0 success，-1 refunded，正数表示pending以及在wait_list中的no
...
constexpr int WaitListT = ((4096 - 5) / (4 + 4 + TrainIDMAXLEN + 1 + 4) - 2) / 2;
constexpr int WaitListL = ((4096 - 8) / (4 + 4 + TrainIDMAXLEN + 1 + sizeof(WaitingOrder)) - 2) / 2;
